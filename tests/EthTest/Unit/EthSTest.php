...
class EthSTest extends TestStatic
{
     ...
     */
    public function testEthSTest__create($abi, $value, $expClass, $expVal)
    {
        $x = new EthS(EthereumStatic::ensureHexPrefix($value));
        $this->assertEquals($expVal, $x->val());
    }

    /**
     * @dataProvider convertDataProvider
     */
    public function testEthSTest__convertHexDataToHexStr($abi, $value, $expClass, $expVal)
    {
        $x = new EthBytes(EthereumStatic::ensureHexPrefix($value));
        $y = $x->convertByAbi($abi);
        $this->assertEquals(EthereumStatic::ensureHexPrefix($value), $y->hexVal());
    }

    /**
     * @dataProvider convertDataProvider
     */
    public function testEthSTest__convertUtf8ToData($abi, $value, $expClass, $expVal)
    {
        $x = new EthS($expVal);
     ...
     * @dataProvider convertDataProvider
     */
    public function testEthSTest__Utf8ToHex($abi, $value, $expClass, $expVal)
    {
        $x = new EthS($expVal);
        $this->assertEquals(EthereumStatic::ensureHexPrefix($value), $x->hexVal());
    }

    /**
     * @dataProvider convertDataProvider
     */
    public function testEthSTest__HexToUtf8($abi, $value, $expClass, $expVal)
    {
        $x = new EthS(EthereumStatic::ensureHexPrefix($value));
        $this->assertEquals($expVal, $x->val());
    }
